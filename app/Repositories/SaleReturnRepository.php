    ...
    public function storeSaleReturn($input):SaleReturn
    {
        try {
            ...
            $saleReturnInputArray = Arr::only($input, [
                ...
                'paid_amount', 'payment_type', 'note', 'date', 'status',
            ]);
            ...
            foreach ($input['sale_return_items'] as $purchaseItem) {
                ...
                $saleExist = SaleItem::where('product_id', $purchaseItem['product_id'])->whereHas('sale',
                    function (Builder $q) use ($input) {
                        $q->where('customer_id', $input['customer_id'])->where('warehouse_id', $input['warehouse_id']);
                    })->exists();
                if ($saleExist) {
                    if ($product) {
            ...
            $search = ['{customer_name}', '{sales_return_id}', '{sales_return_date}', '{sales_return_amount}','{app_name}'];

            $replace = [$customer->name, $saleReturn->reference_code, $saleReturn->date,$saleReturn->grand_total , getSettingValue('company_name')];
