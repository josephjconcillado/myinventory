...
const EditProduct = (props) => {
    ...
    const editSaleUnit = products.length === 1 && units.length >= 0 && units.filter((fill) => fill.id.toString() === products[0].attributes.sale_unit)
    const editPurchaseUnit = products.length === 1 && units.length >= 0 && units.filter((fill) => fill.id.toString() === products[0].attributes.purchase_unit)
    const saleUnitLabel = editSaleUnit ? editSaleUnit.map((ab) => ab.attributes.name) : ''
    const salePurchaseLabel = editPurchaseUnit ? editPurchaseUnit.map((ab) => ab.attributes.name) : ''
    const saleUnitId = editSaleUnit ? editSaleUnit.map((ab) => ab.id) : ''
    const salePurchaseId = editPurchaseUnit ? editPurchaseUnit.map((ab) => ab.id) : ''
    ...
    return (
        <MasterLayout>
            ...
            {products.length === 1 && <ProductForm singleProduct={itemsValue} id={id}/>}
        </MasterLayout>
    )
};
...
export default connect(mapStateToProps, {fetchProduct, productUnitDropdown, fetchUnits})(EditProduct);
