...
const ProductDetail = props => {
    const {products, fetchProduct, isLoading, frontSetting} = props;
    const {id} = useParams();
                            ...
                            <div className='col-xxl-7'>
                                <table className='table table-responsive gy-7'>
                                    ...
                                    <tr>
                                        ...
                                        <td className='py-4'>{frontSetting.value && frontSetting.value.currency_symbol} {product && product.attributes && product.attributes.product_cost}</td>
                                    </tr>
                                    <tr>
                                        <th className='py-4' scope='row'>{getFormattedMessage('product.table.price.column.label')}</th>
                                        <td className='py-4'>{frontSetting.value && frontSetting.value.currency_symbol} {product && product.attributes && product.attributes.product_price}</td>
                                    </tr>
                                    <tr>
                                        <th className='py-4' scope='row'>{getFormattedMessage('product.product-details.unit.label')}</th>
                                        {product && product.attributes && product.attributes.product_unit === '1' &&
                                        <td className='py-4'><span className='badge bg-light-success'><span>Piece</span></span></td>}
                                        {product && product.attributes && product.attributes.product_unit === '2' &&
                                        <td className='py-4'><span className='badge bg-light-primary'><span>Meter</span></span></td>}
                                        {product && product.attributes && product.attributes.product_unit === '3' &&
                                        <td className='py-4'><span className='badge bg-light-warning'><span>Kilogram</span></span>
                                        </td>}
                                    </tr>
                                    <tr>
                                        <th className='py-4' scope='row'>{getFormattedMessage('product.product-details.tax.label')}</th>
                                        <td className='py-4'>{product && product.attributes && product.attributes.order_tax} %</td>
                                    </tr>
                                    <tr>
...
const mapStateToProps = (state) => {
    const {products, isLoading, frontSetting} = state;
    return {products, isLoading, frontSetting}
};
