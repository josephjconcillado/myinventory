...
const RecentSale = (props) => {
    const {recentSales, recentSalesDashboard, frontSetting} = props;
                                ...
                                {recentSalesDashboard && recentSalesDashboard.map((recentSale, index) => {
                                    const renderTooltip = (props) => (
                                        <Tooltip id="button-tooltip" {...props}>
                                            {frontSetting.value && frontSetting.value.currency_symbol} {parseFloat(recentSale.attributes.grand_total).toFixed(2)}
                                        </Tooltip>
                                    );
                                    ...
                                    return (
                                        <tr key={index}>
                                            ...
                                            <td className='py-4'>
                                                ...
                                                >
                                                <span>
                                                    {frontSetting.value && frontSetting.value.currency_symbol} {formatAmount(recentSale.attributes.grand_total)}
                                                </span>
                                                </OverlayTrigger>
                                            </td>
                                            <td className='py-4'>{frontSetting.value && frontSetting.value.currency_symbol} {recentSale.attributes.paid_amount === null ? '0.00' : parseFloat(recentSale.attributes.paid_amount).toFixed(2)}</td>
                                            <td className='py-4'>{frontSetting.value && frontSetting.value.currency_symbol} 0.00</td>
...
const mapStateToProps = (state) => {
    const {recentSalesDashboard} = state;
    return {recentSalesDashboard}
};
