...
const TopSellingProduct = (props) => {
    const {topSelling, topSellingProduct, frontSetting, fetchTopCustomers, topCustomers} = props;
    ...
    return (
        <div className='pt-6'>
            <Row className='g-4'>
                                ...
                                {topSelling && topSelling.map((top, index) => {
                                    const renderTooltip = (props) => (
                                        <Tooltip id="button-tooltip" {...props}>
                                            {frontSetting.value && frontSetting.value.currency_symbol} {parseFloat(top.grand_total).toFixed(2)}
                                        </Tooltip>
                                    );
                                    return (
                                        <tr key={index}>
                                            ...
                                            <td className='py-4'>
                                                ...
                                                >
                                                    <span>
                                                        {frontSetting.value && frontSetting.value.currency_symbol} {formatAmount(top.grand_total)}
                                                    </span>
                                                </OverlayTrigger>
                                            </td>
                                        </tr>
                ...
                <TopCustomersChart frontSetting={frontSetting} topCustomers={topCustomers}/>
            </Row>
        </div>
...
const mapStateToProps = (state) => {
    const {topSelling, topCustomers} = state;
    return {topSelling, topCustomers}
};
