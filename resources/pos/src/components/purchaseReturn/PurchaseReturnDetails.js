...
const PurchaseReturnDetails = (props) => {
    const {fetchPurchaseReturnDetails, purchaseReturnDetails, fetchFrontSetting, frontSetting} = props;
    const {id} = useParams();
                                ...
                                {purchaseReturnDetails.purchase_return_items && purchaseReturnDetails.purchase_return_items.map((details, index) => {
                                    return (
                                        <tr key={index} className='align-middle'>
                                            ...
                                            <td>{frontSetting.value && frontSetting.value.currency_symbol} {parseFloat(details.net_unit_cost).toFixed(2)}</td>
                                            <td>{details.quantity}</td>
                                            <td>{frontSetting.value && frontSetting.value.currency_symbol} {parseFloat(details.product.product_cost).toFixed(2)}</td>
                                            <td>{frontSetting.value && frontSetting.value.currency_symbol} {parseFloat(details.discount_amount).toFixed(2)}</td>
                                            <td>{frontSetting.value && frontSetting.value.currency_symbol} {parseFloat(details.tax_amount).toFixed(2)}</td>
                                            <td>{frontSetting.value && frontSetting.value.currency_symbol} {parseFloat(details.sub_total).toFixed(2)}</td>
                                        </tr>)
                                })}
                                </tbody>
                            </Table>
                                    ...
                                    <div className='table-responsive'>
                                        <table className='table border'>
                                            ...
                                            <tr>
                                                ...
                                                <td className='py-3'>
                                                    {frontSetting.value && frontSetting.value.currency_symbol} {purchaseReturnDetails && purchaseReturnDetails.tax_amount > 0 ? purchaseReturnDetails.tax_amount.toFixed(2) : '0.00'} ({purchaseReturnDetails && parseFloat(purchaseReturnDetails.tax_rate).toFixed(2)}%)
                                                </td>
                                            </tr>
                                            <tr>
                                                <td className='py-3'>{getFormattedMessage('globally.detail.discount')}</td>
                                                <td className='py-3'>{frontSetting.value && frontSetting.value.currency_symbol} {purchaseReturnDetails && parseFloat(purchaseReturnDetails.discount).toFixed(2)}</td>
                                            </tr>
                                            <tr>
                                                <td className='py-3'>{getFormattedMessage('globally.detail.shipping')}</td>
                                                <td className='py-3'>{frontSetting.value && frontSetting.value.currency_symbol} {purchaseReturnDetails && parseFloat(purchaseReturnDetails.shipping).toFixed(2)}</td>
                                            </tr>
                                            <tr>
                                                <td className='py-3 text-primary'>{getFormattedMessage('globally.detail.grand.total')}</td>
                                                <td className='py-3 text-primary'>
                                                    {frontSetting.value && frontSetting.value.currency_symbol} {purchaseReturnDetails && parseFloat(purchaseReturnDetails.grand_total).toFixed(2)}
                                                </td>
                                            </tr>
                                            </tbody>
                                        </table>
...
const mapStateToProps = (state) => {
    const {purchaseReturnDetails, frontSetting} = state;
    return {purchaseReturnDetails, frontSetting}
};
