...
const PurchaseDetails = (props) => {
    const {purchaseDetailsAction, purchaseDetails, fetchFrontSetting, frontSetting} = props;
    const {id} = useParams();
                                    ...
                                    <div className='table-responsive'>
                                        <table className='table border'>
                                            ...
                                            <tr>
                                                ...
                                                <td className='py-3'>
                                                    {frontSetting.value && frontSetting.value.currency_symbol} {purchaseDetails && purchaseDetails.tax_amount > 0 ? purchaseDetails.tax_amount.toFixed(2) : '0.00'} ({purchaseDetails && parseFloat(purchaseDetails.tax_rate).toFixed(2)}%)
                                                </td>
                                            </tr>
                                            <tr>
                                                <td className='py-3'>{getFormattedMessage('globally.detail.discount')}</td>
                                                <td className='py-3'>{frontSetting.value && frontSetting.value.currency_symbol} {purchaseDetails && parseFloat(purchaseDetails.discount).toFixed(2)}</td>
                                            </tr>
                                            <tr>
                                                <td className='py-3'>{getFormattedMessage('globally.detail.shipping')}</td>
                                                <td className='py-3'>{frontSetting.value && frontSetting.value.currency_symbol} {purchaseDetails && parseFloat(purchaseDetails.shipping).toFixed(2)}</td>
                                            </tr>
                                            <tr>
                                                <td className='py-3 text-primary'>{getFormattedMessage('globally.detail.grand.total')}</td>
                                                <td className='py-3 text-primary'>
                                                    {frontSetting.value && frontSetting.value.currency_symbol} {purchaseDetails && parseFloat(purchaseDetails.grand_total).toFixed(2)}
                                                </td>
                                            </tr>
                                            </tbody>
                                        </table>
...
const mapStateToProps = (state) => {
    const {purchaseDetails, frontSetting} = state;
    return {purchaseDetails, frontSetting}
};
