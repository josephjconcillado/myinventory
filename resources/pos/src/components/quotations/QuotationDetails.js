...
const QuotationDetails = (props) => {
    const {quotationDetailsAction, quotationDetails, fetchFrontSetting, frontSetting} = props;
    const {id} = useParams();
                                ...
                                {quotationDetails.quotation_items && quotationDetails.quotation_items.map((details, index) => {
                                    return (
                                        <tr key={index} className='align-middle'>
                                            ...
                                            <td>{frontSetting.value && frontSetting.value.currency_symbol} {parseFloat(details.net_unit_price).toFixed(2)}</td>
                                            <td>{details.quantity}</td>
                                            <td>{frontSetting.value && frontSetting.value.currency_symbol} {parseFloat(details.product_price).toFixed(2)}</td>
                                    ...
                                    <div className='table-responsive'>
                                        <table className='table border'>
                                            ...
                                            <tr>
                                                ...
                                                <td className='py-3'>
                                                    {frontSetting.value && frontSetting.value.currency_symbol} {quotationDetails && quotationDetails.tax_amount > 0 ? quotationDetails.tax_amount.toFixed(2) : '0.00'} ({quotationDetails && parseFloat(quotationDetails.tax_rate).toFixed(2)}%)
                                                </td>
                                            </tr>
                                            <tr>
                                                <td className='py-3'>{getFormattedMessage('globally.detail.discount')}</td>
                                                <td className='py-3'>{frontSetting.value && frontSetting.value.currency_symbol} {quotationDetails && parseFloat(quotationDetails.discount).toFixed(2)}</td>
                                            </tr>
                                            <tr>
                                                <td className='py-3'>{getFormattedMessage('globally.detail.shipping')}</td>
                                                <td className='py-3'>{frontSetting.value && frontSetting.value.currency_symbol} {quotationDetails && parseFloat(quotationDetails.shipping).toFixed(2)}</td>
                                            </tr>
                                            <tr>
                                                <td className='py-3 text-primary'>{getFormattedMessage('globally.detail.grand.total')}</td>
                                                <td className='py-3 text-primary'>
                                                    {frontSetting.value && frontSetting.value.currency_symbol} {quotationDetails && parseFloat(quotationDetails.grand_total).toFixed(2)}
                                                </td>
                                            </tr>
                                            </tbody>
                                        </table>
...
const mapStateToProps = (state) => {
    const {quotationDetails, frontSetting} = state;
    return {quotationDetails, frontSetting}
};
