...
const ProductMainCalculation = (props) => {
    const {inputValues, updateProducts, frontSetting} = props;
                    ...
                    <div className='table-responsive'>
                        <table className='table border'>
                            ...
                            <tr>
                                ...
                                <td className='py-3'>
                                    {frontSetting.value && frontSetting.value.currency_symbol} {taxCal} ({parseFloat(inputValues.tax_rate ? inputValues.tax_rate : 0).toFixed(2)})
                                    %
                                </td>
                            </tr>
                            <tr>
                                <td className='py-3'>{getFormattedMessage('purchase.order-item.table.discount.column.label')}</td>
                                <td className='py-3'>{frontSetting.value && frontSetting.value.currency_symbol} {parseFloat(inputValues.discount ? inputValues.discount : 0).toFixed(2)}</td>
                            </tr>
                            <tr>
                                <td className='py-3'>{getFormattedMessage('purchase.input.shipping.label')}</td>
                                <td className='py-3'>{frontSetting.value && frontSetting.value.currency_symbol} {parseFloat(inputValues.shipping ? inputValues.shipping : 0).toFixed(2)}</td>
                            </tr>
                            <tr>
                                <td className='py-3 text-primary'>{getFormattedMessage('purchase.grant-total.label')}</td>
                                <td className='py-3 text-primary'>
                                    {frontSetting.value && frontSetting.value.currency_symbol} {calculateCartTotalAmount(updateProducts, inputValues)}
                                </td>
                            </tr>
                            </tbody>
                        </table>
