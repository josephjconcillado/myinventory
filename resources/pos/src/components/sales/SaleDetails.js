...
const SaleDetails = (props) => {
    const {saleDetailsAction, saleDetails, fetchFrontSetting, frontSetting} = props;
    const {id} = useParams();
                                    ...
                                    <div className='table-responsive'>
                                        <table className='table border'>
                                            ...
                                            <tr>
                                                ...
                                                <td className='py-3'>
                                                    {frontSetting.value && frontSetting.value.currency_symbol} {saleDetails && saleDetails.tax_amount > 0 ? saleDetails.tax_amount.toFixed(2) : '0.00'} ({saleDetails && parseFloat(saleDetails.tax_rate).toFixed(2)}%)
                                                </td>
                                            </tr>
                                            <tr>
                                                <td className='py-3'>{getFormattedMessage('globally.detail.discount')}</td>
                                                <td className='py-3'>{frontSetting.value && frontSetting.value.currency_symbol} {saleDetails && parseFloat(saleDetails.discount).toFixed(2)}</td>
                                            </tr>
                                            <tr>
                                                <td className='py-3'>{getFormattedMessage('globally.detail.shipping')}</td>
                                                <td className='py-3'>{frontSetting.value && frontSetting.value.currency_symbol} {saleDetails && parseFloat(saleDetails.shipping).toFixed(2)}</td>
                                            </tr>
                                            <tr>
                                                <td className='py-3 text-primary'>{getFormattedMessage('globally.detail.grand.total')}</td>
                                                <td className='py-3 text-primary'>
                                                    {frontSetting.value && frontSetting.value.currency_symbol} {saleDetails && parseFloat(saleDetails.grand_total).toFixed(2)}
                                                </td>
                                            </tr>
                                            </tbody>
                                        </table>
...
const mapStateToProps = (state) => {
    const {saleDetails, frontSetting} = state;
    return {saleDetails, frontSetting}
};
