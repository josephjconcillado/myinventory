...
const SaleReturnDetails = (props) => {
    const {fetchSaleReturnDetails, saleReturnDetails, frontSetting} = props;
    const {id} = useParams();
                                    ...
                                    <div className='table-responsive'>
                                        <table className='table border'>
                                            ...
                                            <tr>
                                                ...
                                                <td className='py-3'>
                                                    {frontSetting.value && frontSetting.value.currency_symbol} {saleReturnDetails && saleReturnDetails.tax_amount > 0 ? saleReturnDetails.tax_amount.toFixed(2) : '0.00'} ({saleReturnDetails && parseFloat(saleReturnDetails.tax_rate).toFixed(2)}%)
                                                </td>
                                            </tr>
                                            <tr>
                                                <td className='py-3'>{getFormattedMessage('globally.detail.discount')}</td>
                                                <td className='py-3'>{frontSetting.value && frontSetting.value.currency_symbol} {saleReturnDetails && parseFloat(saleReturnDetails.discount).toFixed(2)}</td>
                                            </tr>
                                            <tr>
                                                <td className='py-3'>{getFormattedMessage('globally.detail.shipping')}</td>
                                                <td className='py-3'>{frontSetting.value && frontSetting.value.currency_symbol} {saleReturnDetails && parseFloat(saleReturnDetails.shipping).toFixed(2)}</td>
                                            </tr>
                                            <tr>
                                                <td className='py-3 text-primary'>{getFormattedMessage('globally.detail.grand.total')}</td>
                                                <td className='py-3 text-primary'>
                                                    {frontSetting.value && frontSetting.value.currency_symbol} {saleReturnDetails && parseFloat(saleReturnDetails.grand_total).toFixed(2)}
                                                </td>
                                            </tr>
                                            </tbody>
                                        </table>
...
const mapStateToProps = (state) => {
        const {saleReturnDetails, frontSetting} = state;
        return {saleReturnDetails, frontSetting};
};
