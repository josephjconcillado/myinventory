...
const PrintBarcode = () => {
    const {warehouses, products, purchaseProducts, customProducts = preparePurchaseProductArray(products)} = useSelector(state => state)
            ...
            <div className='card card-body'>
                ...
                <div className='col-12 md-12'>
                    <Table responsive>
                        ...
                        <tbody>
                        {printBarcodeValue.warehouse_id && updateProducts ?
                            updateProducts.map((singleProduct, index) => {
                                return (
                                    <>
                                        <PrintTable singleProduct={singleProduct} index={index}
                ...
                {updateProducts ? updateProducts.map((product, index) => {
                        return <BarcodeShow product={product} index={index}
                                            paperSize={printBarcodeValue.paperSizeValue} updated={updated}/>
                    }) : ''}
            </div>
        </MasterLayout>
    )
