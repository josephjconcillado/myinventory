...
const StockReport = (props) => {
    ...
    const itemsValue = currencySymbol && stockReports.length >= 0 && stockReports.map(stockReport => ({
        ...
        product_unit: stockReport.attributes.product.product_unit,
    ...
    const columns = [
        ...
        },
        {
            ...
            selector: row => row.currency + ' ' + parseFloat(row.product_cost).toFixed(2),
            sortField: 'product_cost',
            sortable: true,
        },
        {
            name: getFormattedMessage('product.table.price.column.label'),
            selector: row => row.currency + ' ' + parseFloat(row.product_price).toFixed(2),
...
const mapStateToProps = (state) => {
    const {isLoading, totalRecord, warehouses, frontSetting, stockReports} = state;
    return {isLoading, totalRecord, warehouses, frontSetting, stockReports}
};
