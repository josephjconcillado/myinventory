...
const StockReport = (props) => {
    ...
    const itemsValue = productQuantityReport && productQuantityReport.length >= 0 && productQuantityReport.map(quantityReport => ({
        ...
        product_unit: quantityReport && quantityReport.product_unit,
    }));
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: row => {
                ...
                {row.product_unit === '1' && <span className='badge bg-light-success me-2'><span>{getFormattedMessage("unit.filter.piece.label")}</span></span>
                || row.product_unit === '2' && <span className='badge bg-light-primary me-2'><span>{getFormattedMessage("unit.filter.meter.label")}</span></span>
                || row.product_unit === '3' && <span className='badge bg-light-warning me-2'><span>{getFormattedMessage("unit.filter.kilogram.label")}</span></span>}
            </div>
            }
        },
        {
