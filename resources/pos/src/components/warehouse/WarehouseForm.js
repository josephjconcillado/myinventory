...
const WarehouseForm = (props) => {
    ...
    const disabled =
        singleWarehouse &&
        singleWarehouse[0].name === warehouseValue.name &&
        singleWarehouse[0].phone === warehouseValue.phone &&
        singleWarehouse[0].country === warehouseValue.country &&
        singleWarehouse[0].city === warehouseValue.city &&
        singleWarehouse[0].email === warehouseValue.email &&
        singleWarehouse[0].zip_code === warehouseValue.zip_code;
    ...
    const handleValidation = () => {
        ...
        if (!warehouseValue["name"]) {
            ...
            errorss["zip_code"] = getFormattedMessage(
                "warehouse.input.zip-code.validate.label"
            );
        } else if (
            warehouseValue["zip_code"] &&
            warehouseValue["zip_code"].length < 4
        ) {
            errorss["zip_code"] = getFormattedMessage(
                "warehouse.input.zip-code.valid.validate.label"
            );
        } else {
            isValid = true;
        }
                ...
                <Form>
                    <div className="row">
                        ...
                        <div className="col-md-6 mb-3">
                            <label className="form-label">
                                {getFormattedMessage(
                                    "warehouse.input.zip-code.label"
                                )}
                                :
                            </label>
                            <span className="required" />
                            <input
                                ...
                                placeholder={placeholderText(
                                    "warehouse.input.zip-code.placeholder.label"
                                )}
                                onChange={(e) => onChangeInput(e)}
                                onKeyPress={(event) => numValidate(event)}
                            />
                            <span className="text-danger d-block fw-400 fs-small mt-2">
                                {errors["zip_code"] ? errors["zip_code"] : null}
                            </span>
                        </div>
                        <ModelFooter
