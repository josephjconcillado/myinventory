        ...
        >
            ...
            <Modal.Body>
                            ...
                            <div className='product-border'>
                                <tr className='border-0'>
                                    ...
                                    <td className='text-end w-125px'>
                                        {currency} {(productName.quantity * (calculateProductCost(productName))).toFixed(2)}
                                    </td>
                                </tr>
                            </div>
                            </tbody>
                ...
                <div className='d-flex product-border'>
                    ...
                    <div
                        className='text-end ms-auto'> {currency} {updateProducts.subTotal ? updateProducts.subTotal.toFixed(2) : '0.00'}
                    </div>
                </div>
                ...
                <div className='d-flex product-border'>
                    ...
                    <div
                        className='text-end ms-auto'> {currency} {updateProducts.taxTotal ? updateProducts.taxTotal.toFixed(2) : '0.00'} ({updateProducts ? parseFloat(updateProducts.tax).toFixed(2) : '0.00'} %)
                    </div>
                </div>
                <div className='d-flex product-border'>
                    <div>{getFormattedMessage('purchase.order-item.table.discount.column.label')}:</div>
                    <div
                        className='text-end ms-auto'> {currency} {updateProducts ? parseFloat(updateProducts.discount).toFixed(2) : '0.00'}
                    </div>
                </div>
                {
                    updateProducts.shipping ? <div className='d-flex product-border'>
                        <div>Shipping:</div>
                        <div
                            className='text-end ms-auto'> {currency} {updateProducts ? parseFloat(updateProducts.shipping).toFixed(2) : '0.00'}
                        </div>
                    </div> : ''
                }
                <div className='d-flex product-border mb-4'>
                    <div>{getFormattedMessage("purchase.grant-total.label")}:</div>
                    <div className='text-end ms-auto'> {currency} {updateProducts.grandTotal}</div>
                </div>
                <Table striped className='mb-7'>
                    ...
                    <tr>
                        ...
                        <td className='text-end py-2'>
                            {currency} {updateProducts.grandTotal}</td>
                    </tr>
                    </tbody>
                </Table>
