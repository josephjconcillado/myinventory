...
const CashPaymentModel = (props) => {
    const {
        ...
        paymentTypeDefaultValue, paymentTypeFilterOptions
    } = props;
                            ...
                            <div className='card-body p-6'>
                                <Table striped bordered hover className='mb-0'>
                                    ...
                                    <tr>
                                        ...
                                        <td className="px-3"    >{settings.attributes && settings.attributes.currency_symbol} {subTotal ? subTotal.toFixed(2) : '0.00'}</td>
                                    </tr>
                                    <tr>
                                        <td scope='row' className="ps-3">{getFormattedMessage("globally.detail.order.tax")}</td>
                                        <td className="px-3">{settings.attributes && settings.attributes.currency_symbol} {taxTotal ? taxTotal.toFixed(2) : '0.00'} ({cartItemValue.tax ? parseFloat(cartItemValue.tax).toFixed(2) : '0.00'} %)</td>
                                    </tr>
                                    <tr>
                                        <td scope='row' className='ps-3'>{getFormattedMessage('purchase.order-item.table.discount.column.label')}</td>
                                        <td className="px-3">{settings.attributes && settings.attributes.currency_symbol} {cartItemValue.discount ? parseFloat(cartItemValue.discount).toFixed(2) : '0.00'}</td>
                                    </tr>
                                    <tr>
                                        <td scope='row' className='ps-3'>{getFormattedMessage("purchase.input.shipping.label")}</td>
                                        <td className="px-3">{settings.attributes && settings.attributes.currency_symbol} {cartItemValue.shipping ? parseFloat(cartItemValue.shipping).toFixed(2) : '0.00'}</td>
                                    </tr>
                                    <tr>
                                        <td scope='row' className='ps-3'>{getFormattedMessage("purchase.grant-total.label")}</td>
                                        <td className="px-3">{settings.attributes && settings.attributes.currency_symbol} {grandTotal}</td>
                                    </tr>
                                    </tbody>
                                </Table>
                            </div>
