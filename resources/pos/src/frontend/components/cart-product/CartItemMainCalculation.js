...
const CartItemMainCalculation = (props) => {
    const {totalQty, subTotal, cartItemValue, onChangeCart, grandTotal, frontSetting} = props;
        ...
        <div className='calculation mt-5'>
            <Row className='total-price'>
                ...
                <div className='col-6 d-flex flex-column justify-content-center align-items-end mb-2'>
                    ...
                    <h4 className='fs-3 mb-2 text-gray-600'>{getFormattedMessage('pos.subtotal.small.title')} : {frontSetting.value && frontSetting.value.currency_symbol} {subTotal ? (subTotal).toFixed(2) : '0.00'}</h4>
                    <h2 className='fs-1 mb-2 text-gray-800'>{getFormattedMessage('pos-total.title')} : {frontSetting.value && frontSetting.value.currency_symbol} {grandTotal ? grandTotal : '0.00'}</h2>
                </div>
            </Row>
        </div>
    )
