...
const PosMainPage = (props) => {
    ...
    const [cashPaymentValue, setCashPaymentValue] = useState({notes: ''});
    ...
    const onCashPayment = (event) => {
        ...
        if (valid) {
            posCashPaymentAction(prepareData(updateProducts), setUpdateProducts, setModalShowPaymentSlip, posAllProduct, {brandId, categoryId, selectedOption});
    ...
    const loadPaymentSlip = () => {
        return (
            <div className='d-none'>
                <PaymentSlipModal allConfigData={allConfigData} paymentDetails={paymentDetails} updateProducts={paymentPrint} printPaymentReceiptPdf={printPaymentReceiptPdf}
                                  setModalShowPaymentSlip={setModalShowPaymentSlip} frontSetting={frontSetting}
                                  modalShowPaymentSlip={modalShowPaymentSlip} paymentType={paymentValue.pyment_type.label}/>
            </div>
        )
    };
    ...
    return (
        <Container className='pos-screen px-3' fluid>
                    ...
                    <div className='left-content custom-card mb-3 p-3'>
                         <div className='main-table overflow-auto'>
                             <Table className="mb-0">
                                 ...
                                 <tr>
                                     <th style={{ minWidth: '164px' }}>{getFormattedMessage("pos-product.title")}</th>
                                     <th style={{ minWidth: '118px' }}>{getFormattedMessage('pos-qty.title')}</th>
                                     <th style={{ minWidth: '140px' }}>{getFormattedMessage('pos-price.title')}</th>
                                     <th style={{ minWidth: '135px' }} colSpan='2'>{getFormattedMessage('pos-sub-total.title')}</th>
                                 </tr>
                                 </thead>
                                 <tbody className='border-0'>
                                 {updateProducts && updateProducts && updateProducts.map((updateProduct, index) => {
                                     return <ProductCartList singleProduct={updateProduct} index={index} posAllProducts={posAllProducts}
                                                             ...
                                                             frontSetting={frontSetting} newCost={newCost}
                         ...
                         <CartItemMainCalculation totalQty={totalQty} subTotal={subTotal} grandTotal={grandTotal}
                                                  ...
                                                  frontSetting={frontSetting}/>
                        <PaymentButton updateProducts={updateProducts} setUpdateProducts={setUpdateProducts} setCartItemValue={setCartItemValue} setCashPayment={setCashPayment}
                                           cartItemValue={cartItemValue} grandTotal={grandTotal} subTotal={subTotal}/>
                    </div>
                </Col>
                <Col lg={7} xxl={8} xs={12} className='ps-lg-0'>
                    <div className='right-content mb-3'>
                        <div className="d-sm-flex align-items-center">
                            <ProductSearchbar customCart={customCart} setUpdateProducts={setUpdateProducts} updateProducts={updateProducts}
                                // handleOnSelect={handleOnSelect} handleOnSearch={handleOnSearch}
                                              // searchString={searchString}
                            />
                            <HeaderAllButton onClickFullScreen={onClickFullScreen} opneCalculator={opneCalculator} setOpneCalculator={setOpneCalculator}/>
                        </div>
            ...
            <CashPaymentModel cashPayment={cashPayment} totalQty={totalQty} cartItemValue={cartItemValue}
                              ...
                              paymentTypeDefaultValue={paymentTypeDefaultValue} paymentTypeFilterOptions={paymentTypeFilterOptions}
                              />
            }
        </Container>
    )
...
const mapStateToProps = (state) => {
    const {posAllProducts, frontSetting, settings, cashPayment, allConfigData} = state;
    return {posAllProducts, frontSetting, settings, paymentDetails: cashPayment, customCart: prepareCartArray(posAllProducts), allConfigData}
};
